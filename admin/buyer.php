...
if(isset($_POST['delete'])){
   ...
   $verify_delete = $conn->prepare("SELECT * FROM `buyer` WHERE id = ?");
   $verify_delete->execute([$delete_id]);

   if($verify_delete->rowCount() > 0){
      $delete_bookings = $conn->prepare("DELETE FROM `buyer` WHERE id = ?");
...
<section class="grid">
   ...
   <?php
      if(isset($_POST['search_box']) OR isset($_POST['search_btn'])){
         ...
         $select_messages = $conn->prepare("SELECT * FROM `buyer` WHERE p_name LIKE '%{$search_box}%' OR name LIKE '%{$search_box}%' OR number LIKE '%{$search_box}%' OR email LIKE '%{$search_box}%'");
         $select_messages->execute();
      }else{
         $select_messages = $conn->prepare("SELECT * FROM `buyer`");
         $select_messages->execute();
      }
   ...
   ?>
   <div class="box">
      <p>p_name : <span><?= $fetch_messages['p_name']; ?></span></p>
